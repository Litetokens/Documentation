#  LITETOKENS HTTP Gateway interface

# Demo of the conversion between base58check and hexString
java:
https://github.com/litetokens/wallet-cli/blob/master/src/main/java/org/litetokens/demo/TransactionSignDemo.java#L92

php:
https://github.com/litetokens/Documentation/blob/master/XLT_CN/index.php

Available in the latest release of java-litetokens. 
...
Parameters: passPhrase is the password, converted from ascii to hex. toAddress is the recipient address, converted into a hex string; amount is the amount of XLT to transfer expressed in SUN.
...
Parameters：passPhrase is the private key in hex string format. toAddress is the recipient address, converted into a hex string; amount is the amount of XLT to transfer in SUN.
...
Function：Freezes an amount of XLT. Will give bandwidth OR Energy and LITETOKENS Power(voting rights) to the owner of the frozen tokens.
...
Parameters：
owner_address is the address that is freezing xlt account，converted to a hex string
frozen_balance is the number of frozen xlt
...
Return value：Freeze xlt transaction raw data

/wallet/unfreezebalance
Function：Unfreeze XLT that has passed the minimum freeze duration. Unfreezing will remove bandwidth and LITETOKENS Power.
demo：curl -X POST http://127.0.0.1:8090/wallet/unfreezebalance -d '{
"owner_address":"41e472f387585c2b58bc2c9bb4492bc1f17342cd1",
}'
Parameters：
owner_address address to unfreeze XLT for，converted to a hex string
Return value：Unfreeze XLT transaction raw data
...
Return value：Withdraw XLT transaction raw data
...
Parameters：
  ...
  fee_limit：Maximum XLT consumption, measured in SUN（1XLT = 1,000,000SUN）
  call_value：Amount of XLT transferred with this transaction, measured in SUN（1XLT = 1,000,000SUN）
