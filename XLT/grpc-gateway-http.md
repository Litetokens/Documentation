...
Function: creates token; on Litetokens’s public blockchain, users can issue tokens which can be transferred reciprocally or participate in token offerings with their XLT. During token creation, an issuer can chose to freeze a certain amount of tokens.
...
Function: to participate in token offerings, users can exchange for issued tokens with XLT.
...
Function: Freezes XLT for the account
Parameters: owner_address should be in base64 format; frozen_balance is the amount of frozen XLT in sun; frozen_duration is the frozen period.
...
Function:  Unfreezes XLT for the account
...
XLT easy transfer: /wallet/easytransfer
...
Parameters: passPhrase is the password; toAddress is the recipient address; amount is the amount of XLT to transfer.
Warning: Please control risks when using this API. To ensure environmental security, please do not invoke APIs provided by other or invoke this very API on a public network.
