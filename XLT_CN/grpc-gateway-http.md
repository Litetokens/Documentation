...
需要部署grpc-gateway，详情请见：[grpc-gateway文档](https://github.com/litetokens/grpc-gateway/blob/master/README.md)

grpc-gateway会自动把proto中的bytes字段转换为base64编码。所以如果输入参数是bytes类型，需要首先转化为base64编码，再发送请求；如果输出参数是bytes类型，请用base64解码该参数，再做后续处理。 Litetokens提供了一个编解码的工具，下载地址是：https://github.com/litetokens/litetokens-demo/raw/master/LitetokensConvertTool.zip
...
作用： 发行Token，用户可以再Litetokens公链上发行Token，Token可以被相互转账，可以用Trx参与。用户在发行Token的时候，可以选择冻结部分Token。
参数说明：
...
参数说明：owner_address需要是base64格式，frozen_balance是drop形式的冻结xlt金额，frozen_duration是冻结天数

解冻xlt：/wallet/unfreezebalance
...
参数说明：passPhrase是用户密码，toAddress是转账接收地址，amount是转账xlt数量
警告：使用本接口请控制风险。保证环境的安全，不要调用他人提供的API，不要在公共网路上调用本接口。
