...
当前版本是3秒。
## 1.5 交易模型
基于账号模型，目前不支持UTXO模型。XLT的最小单位是Drop，1XLT = 1000000Drop。目前仅支持1对1交易，尚不支持1对n或者n对1转账。
## 1.6 账号模型
...
# 2 LITETOKENS网络结构
## 2.1 节点类型
LITETOKENS主链网络中有三种类型的节点，分别是witness、fullnode和solidity node。
...
mainnet浏览器https://scan.litetokens.co
testnet浏览器https://test.scan.litetokens.co
请交易所在testnet上进行测试。
testnet的配置请参照https://github.com/litetokens/LitetokensDeployment/blob/master/test_net_config.conf
mainnet的配置请参考https://github.com/litetokens/LitetokensDeployment/blob/master/main_net_config.conf
