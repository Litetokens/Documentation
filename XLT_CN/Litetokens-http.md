...
作用：创建账号，一个已经激活的账号创建一个新账号，需要花费0.1xlt
...
参数说明：passPhrase是用户密码，需要是hexString格式；toAddress是转入地址，需要是hexString格式；amount是转账xlt数量
...
参数说明：
owner_address是冻结xlt账号的地址，需要是hexString格式
frozen_balance是冻结xlt的数量
frozen_duration是冻结天数，最少是3天
resource: 冻结xlt获取资源的类型(可以是BANDWIDTH或者ENERGY，BANDWIDTH为带宽，ENERGY为虚拟机消耗资源)
返回值：冻结xlt的transaction

wallet/unfreezebalance
作用：解冻已经结束冻结期的xlt，会同时失去这部分xlt带来的带宽和投票权
demo：curl -X POST http://127.0.0.1:8090/wallet/unfreezebalance -d '{
"owner_address":"41e472f387585c2b58bc2c9bb4492bc1f17342cd1",
"resource": "BANDWIDTH"
}'
参数说明：
owner_address是解冻xlt账号的地址，需要是hexString格式
resource可以是BANDWIDTH或者ENERGY
返回值：解冻xlt的transaction
...
fee_limit：最大消耗的SUN（1XLT = 1,000,000SUN）
call_value：本次调用往合约转账的SUN（1XLT = 1,000,000SUN）
