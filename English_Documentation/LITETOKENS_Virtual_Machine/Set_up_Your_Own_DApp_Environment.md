...
Make sure that the private chain in the prerequisite has been successfully deployed by checking FullNode/logs/litetokens.log and see if log message "Produce block successfully" of persistent block generation appears.
...
The compiler of LITETOKENS is slightly different from that of Ethereum and is still integrating with Remix. Therefore, we are providing a temporary way of acquiring ABI and ByteCode instead of acquiring them directly from Remix. Copy the code above to SImpleWebCompiler and click the Compile button to attain ABI and ByteCode.
