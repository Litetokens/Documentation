...
24小时内，LITETOKENS网络提供的总CPU资源为50_000_000_000Energy。   

### <h2 id="2.1">2.1 Energy的获取</h2>

Energy只能通过冻结XLT获取，获取的Energy额度 = 为获取Energy冻结的XLT / 整个网络为获取Energy冻结的XLT 总额 * 50_000_000_000。
也就是所有用户按冻结XLT平分固定额度的Energy。

例如，假设当前网络中为了Energy而冻结的XLT总量为1000_000_000XLT，一个账户冻结了1000XLT，占总冻结量的百万分之一，即32400微秒。    
一条合约执行如果需要花费324微秒，那么用户可以触发100次该合约。

**注意** 
- 由于网络中总冻结量以及账户的冻结量随时可能发生变化，因此账户拥有的CPU资源不是固定值。 
- 冻结资金不可同时获得Bandwidth Points 与Energy，即如果指定冻结XLT获得bandwidth，Energy不会变化。
...
Energy的消耗规则如下：
    ...
    1、根据智能合约中设定的百分比参数，扣除智能合约所有者通过冻结XLT获取的的Energy，不足的部分由调用者提供。
    
    2、尝试消耗交易发起者冻结获取的Energy。如果不足，有多少扣多少，然后进入下一步。

    3、尝试消耗交易发起者的XLT，消耗的XLT = 所需Energy * 100 sun。
